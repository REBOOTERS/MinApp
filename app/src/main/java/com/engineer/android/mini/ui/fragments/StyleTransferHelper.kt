...
class StyleTransferHelper(
    ...
    init {
        if (setupStyleTransfer()) {
            ...
            inputTransformTargetHeight = interpreterTransform!!.getInputTensor(0).shape()[1]
            inputTransformTargetWidth = interpreterTransform!!.getInputTensor(0).shape()[2]
...
//                }
    ...
    fun transfer(bitmap: Bitmap) {
        ...
        processInputImage(
            bitmap, inputTransformTargetWidth, inputTransformTargetHeight
        )?.let { inputImage ->
            processInputImage(
                styleImage!!, inputPredictTargetWidth, inputPredictTargetHeight
            )?.let { styleImage ->
                ...
                val transformInput = arrayOf(inputImage.buffer, predictOutput.buffer)
    ...
    private fun processInputImage(
        image: Bitmap, targetWidth: Int, targetHeight: Int
    ): TensorImage? {
        ...
        val imageProcessor =
            ImageProcessor.Builder().add(ResizeWithCropOrPadOp(cropSize, cropSize)).add(
                ResizeOp(
                    targetHeight, targetWidth, ResizeOp.ResizeMethod.BILINEAR
                )

...
fun createWorkRequest(userId: String?): PeriodicWorkRequest {
    val constraints = Constraints.Builder().setRequiredNetworkType(NetworkType.UNMETERED)
        .setRequiresCharging(true).setRequiresBatteryNotLow(true).build()


    return PeriodicWorkRequestBuilder<UploadUserLogWork>(6, TimeUnit.MINUTES).setInputData(
        workDataOf(
            INPUT_TAG to userId
        )
    ).setConstraints(constraints).setBackoffCriteria(BackoffPolicy.LINEAR, 3, TimeUnit.SECONDS)
...
object WorkUtil {
    ...
    fun clean(context: Context, path: String) {
        val request = OneTimeWorkRequestBuilder<CleanWork>().addTag("cleanWork")
